...
def update_regulatory_documents(service_doc: ASCService, json_reg_docs):
	...
	for reg_doc in service_doc.regulatory_documents:
		reg_doc_name = frappe.get_value("ASC Service Regulatory Doc",reg_doc.reg_doc, "title" )
		if reg_doc_name:
			exist_reg_doc.append(reg_doc_name)
	for reg_doc_json in json_reg_docs:
		if not reg_doc_json in exist_reg_doc:
			reg_doc_db_name = frappe.get_value("ASC Service Regulatory Doc", {
				"title": reg_doc_json}, "name")
			if not reg_doc_db_name:
				reg_doc_db = frappe.new_doc("ASC Service Regulatory Doc")
				reg_doc_db.insert()
				print("new reg_doc_db")
			else:
				reg_doc_db = frappe.get_doc("ASC Service Regulatory Doc",reg_doc_db_name)
				
			reg_doc_db.title = reg_doc_json
			if reg_doc_db.has_value_changed:
				reg_doc_db.save()
			service_doc.append("regulatory_documents", {
				"reg_doc": reg_doc_db.name})


def update_produces(service_doc: ASCService, json_produces):
	exist_produces = []
	for produce in service_doc.produces:
		exist_produces.append(produce.get("title"))
	for produce in json_produces:
		if not produce in exist_produces:
			service_doc.append("produces", {
				"title": produce})
...
def update_cost(service_doc: ASCService, json_variations):
	for variation in json_variations:
		cost_doc_in_db = frappe.get_value(
			"ASC Cost", {"identifier": variation["identifier"], "service": service_doc.name}, "name")
